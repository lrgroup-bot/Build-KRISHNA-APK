param([string[]]$Roots = @(),[int]$MaxDepth = 5,[string]$OutputPath = "$env:USERPROFILE\.krishna\projects.json")
...
function Get-Depth([string]$root,[string]$path) {
  $base = $root.TrimEnd('\')
  if ($path.Length -le $base.Length) { return 0 }
  $rel = $path.Substring($base.Length).Trim('\')
  if (-not $rel) { return 0 }
...
foreach ($root in $Roots) {
  ...
  Get-ChildItem -LiteralPath $root -Directory -Force -Recurse -ErrorAction SilentlyContinue |
    ...
    ForEach-Object {
      $remote = ""
      try { $remote = (& git -C $_.FullName config --get remote.origin.url 2>$null | Out-String).Trim() } catch {}
