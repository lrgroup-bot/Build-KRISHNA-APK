        ...
        if self.path in ("/v1/chat", "/api/core/chat"):
            ...
            try:
                ...
                if mode == "chat":
                    ...
                    prefix = (
                        "KARMA managed-work mode. Turn the request into a bounded plan, identify evidence and verification criteria, "
                        "and do not claim execution unless a registered action produced evidence. "
                        if mode == "karma" else
                        "VISHWAKARMA software-work mode. Treat this as an authorized coding/project request: inspect project context, "
                        "propose the smallest safe implementation path, use shadow/verification gates for mutations, and report concrete diffs/tests. "
                    )
